...
function createBricks() {
  ...
  for (var k = 0; k < 8; k++) {
    ...
    for (var i = 0; i < 12; i++) {
      ...
      brick.cx += brick.halfWidth * 2 * i + 4*i;
      bricks.push(brick);
    }
...
// Text appears when user wins game
function gWon(ctx){
  ctx.save();
  ctx.font = "70px Arial";
  ctx.shadowBlur = 20;
  ctx.shadowColor = "#FAFA37";
  ctx.fillStyle = "white";
  ctx.fillText("YOU WON!", 280,350);
  ctx.restore();
}

// Text appears when user loses game
function gOver(ctx){
  ctx.save();
  ctx.font = "70px Arial";
  ctx.shadowBlur = 20;
...
function renderSimulation(ctx) {
    ...
    //Song that plays throughout game
    gameSong.play();

    // Play sound when game is over and display on screen
    if (g_main._isGameOver === true) {
      gameOver.play();
      gOver(g_ctx);
      gameSong.pause();
    }
    // Play sound when game is won and display on screen
    if (g_main._isGameWon === true) {
      gameWin.play();
      gWon(g_ctx);
      gameSong.pause();
    }
