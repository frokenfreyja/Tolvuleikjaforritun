...
Sprite.prototype.drawWrappedCentredAt = function (ctx, cx, cy, rotation) {
   ...
   this.drawWrappedHelper(ctx, cx, cy, rotation);
};

Sprite.prototype.drawWrappedHelper = function (ctx, cx, cy, rotation) {
   if (cx + this.halfWidth > 400) {
   		this.drawCentredAt(ctx, cx - 400, cy, rotation);
   }
   if (cx - this.halfWidth < 0) {
   		this.drawCentredAt(ctx, cx + 400, cy, rotation);
   }
   if (cy + this.halfHeight > 400) {
   		this.drawCentredAt(ctx, cx, cy - 400, rotation);
   }
   if (cy - this.halfHeight < 0) {
   		this.drawCentredAt(ctx, cx, cy + 400, rotation);
   }
...
Ship.prototype.computeGravity = function () {
    ...
    // YOUR STUFF HERE
    if (!g_keys[KEY_HALT] && g_useGravity) return NOMINAL_GRAVITY; 
...
Ship.prototype.applyAccel = function (accelX, accelY, du) {
    ...
    // YOUR STUFF HERE
    var velFinalX = this.velX + accelX * du;
    var velFinalY = this.velY + accelY * du;
    // Average velocity
    var velAvgX = (this.velX + velFinalX) / 2;
    var velAvgY = (this.velY + velFinalY) / 2;
		
    var nextX = velAvgX * du;
    var nextY = velAvgY * du;
		// Bounce of top and bottom of playfield
    if (g_useGravity) {
    	if (this.cy + g_shipSprite.halfWidth + nextY > 400 ||
      		this.cy - g_shipSprite.halfWidth + nextY < 0) {
      	this.cx += nextX;
				this.cy -= nextY;
        this.velX = velFinalX;
   		  this.velY = -velFinalY * 0.9;
...
Ship.prototype.wrapPosition = function () {
    ...
    // x coordinates
    if (this.cx < 0) {
    		this.cx = 400 - Math.abs(this.cx) % 400;
    } else {
    		this.cx = Math.abs(this.cx) % 400;
    }
    // y coordinates
    if (this.cy < 0) {
    		this.cy = 400 - Math.abs(this.cy) % 400;
    } else {
    		this.cy = Math.abs(this.cy) % 400;
    }
